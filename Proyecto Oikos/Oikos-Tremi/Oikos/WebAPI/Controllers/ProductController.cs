...
namespace WebAPI.Controllers {
    public class ProductController : ApiController {
        ...
        */
        ...
        [HttpGet]
        public IHttpActionResult RetrieveAllByCategory(int catId) {
            apiResp = new ApiResponse();
            var mng = new MasterManager();
            var prodLst = new List<Product>();
            var catLst = mng.RetrieveAll<ProductsCategories>(EntityTypes.ProductsCategories);
            foreach (var cat in catLst)
                if (cat.CategoryId == catId)
                    prodLst.Add(mng.Retrieve<Product>(new Product {ProductId = cat.ProductId},
                        EntityTypes.Product));
